...
public class AssociationDetection {
	...
	/** 如果field在associationList里面，返回true */
	...
	/** 检测一个类都在哪里被当做成员变量使用，把这样的关系都添加到associationList */
	private void generateAssociations(SystemObject systemObject) {
        ...
        while(classIt.hasNext()) {
            ...
            while(fieldIt.hasNext()) {
                ...
                // 如果是association类的话，这儿定义的类可以参考构造函数中添加的值（cover also other collections in the future）
				if(acceptableOriginClassNames.contains(type)) {
					// 获取范式类型，就是类似利用多态性获得描述信息
					String genericType = fieldObject.getType().getGenericType();
					if(genericType != null) {
						// 遍历所有自定义的class，如果当前变量的类型为自定义的话记为一个association，并添加到associationList
				...
				// 不是association类，比如exception，activator之类的
	...
	/** 对无法用genericType获得名称的采用以下方法获得association */
